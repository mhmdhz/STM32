...
HardwareSerial Serial3(PD_9, PD_8);   //Connected to raspbbery Pi to serial comunicate TX3,RX3
...
void setup() {
  ...
  SERIAL_PORT.println("Connection faild  to ELM327");
  }
  for (int i = 0; i < 20; i++) {
    digitalWrite(Led, 1);
    delay(100);
  ...
  // Optionally check battery charging status using AT+CBC
  SERIAL_PORT.println("Checking battery status...");
  modem.sendAT("+CBC");  // Get battery status
  String batteryStatus = modem.stream.readStringUntil('\n');
