...
extern int vehicleSpeedValue;
extern int engineRpmValue;
extern int engineCoolantTemp;
extern int intakeAirTempValue;
extern int throttlePositionValue;
extern int timingAdvanceValue;
extern int engineLoadValue;
extern float mafAirFlowRate;

void read_K() {
  // getSupportedPIDs(0x01);
  // SERIAL_DEBUG.println("Suported LiveData: ");
  // for (int i = 0; i < 32; i++) {
  //   SERIAL_DEBUG.print(supportedLiveData[i]);
  //   SERIAL_DEBUG.print(", ");
  // }
  // SERIAL_DEBUG.println();

  // getSupportedPIDs(0x02);
  // SERIAL_DEBUG.println("Suported FreezeFrame: ");
  // for (int i = 0; i < 32; i++) {
  //   SERIAL_DEBUG.print(supportedFreezeFrame[i]);
  //   SERIAL_DEBUG.print(", ");
  // }
  // SERIAL_DEBUG.println();

  // getSupportedPIDs(0x09);
  // SERIAL_DEBUG.println("Suported VehicleInfo: ");
  // for (int i = 0; i < 32; i++) {
  //   SERIAL_DEBUG.print(supportedVehicleInfo[i]);
  //   SERIAL_DEBUG.print(", ");
  // }
  // SERIAL_DEBUG.println();

  //SERIAL_DEBUG.println("Live Data: ");
  delay(200);
  getPID(VEHICLE_SPEED);
  SERIAL_DEBUG.print("Speed: "), SERIAL_DEBUG.println(vehicleSpeedValue);
  delay(200);
  getPID(ENGINE_RPM);  
  SERIAL_DEBUG.print("Engine RPM: "), SERIAL_DEBUG.println(engineRpmValue);
  delay(200);
  getPID(ENGINE_COOLANT_TEMP);
  SERIAL_DEBUG.print("Coolant Temp: "), SERIAL_DEBUG.println(engineCoolantTemp);
  delay(200);
  getPID(INTAKE_AIR_TEMP);
  SERIAL_DEBUG.print("Intake Air Temp: "), SERIAL_DEBUG.println(intakeAirTempValue);
  delay(200);
  getPID(THROTTLE_POSITION);
  SERIAL_DEBUG.print("Throttle: "), SERIAL_DEBUG.println(throttlePositionValue);
  delay(200);
  getPID(TIMING_ADVANCE);
  SERIAL_DEBUG.print("Timing Advance: "), SERIAL_DEBUG.println(timingAdvanceValue);
  delay(200);
  getPID(ENGINE_LOAD);
  SERIAL_DEBUG.print("Engine Load: "), SERIAL_DEBUG.println(engineLoadValue);
  delay(200);
  getPID(MAF_FLOW_RATE);
  SERIAL_DEBUG.print("MAF Flow Rate: "), SERIAL_DEBUG.println(mafAirFlowRate);

  // get_DTCs();
  // SERIAL_DEBUG.println("DTCs: ");
  // for (int i = 0; i < 32; i++) {
  //   SERIAL_DEBUG.print(dtcBuffer[i]);
  //   SERIAL_DEBUG.print(", ");
  // }
  // SERIAL_DEBUG.println();

  // SERIAL_DEBUG.println("Freeze Frame: ");
  // getFreezeFrame(VEHICLE_SPEED);
  // SERIAL_DEBUG.print("Speed: "), SERIAL_DEBUG.println(freeze_SPEED);
  // getFreezeFrame(ENGINE_RPM);
  // SERIAL_DEBUG.print("Engine RPM: "), SERIAL_DEBUG.println(freeze_RPM);
  // getFreezeFrame(ENGINE_COOLANT_TEMP);
  // SERIAL_DEBUG.print("Coolant Temp: "), SERIAL_DEBUG.println(freeze_COOLANT_TEMP);
  // getFreezeFrame(ENGINE_LOAD);
  // SERIAL_DEBUG.print("Engine Load: "), SERIAL_DEBUG.println(freeze_ENGINELOAD);
...
bool init_OBD2() {
  ...
  if (protocol == "Automatic" || protocol == "ISO14230_Slow" || protocol == "ISO9141") {
    ...
    if (resultBuffer[0] == 0x55) {
      if (resultBuffer[1] == resultBuffer[2]) {
        SERIAL_DEBUG.println("Your Protocol is ISO9141");
        protocol = "ISO9141";
      } else {
        SERIAL_DEBUG.println("Your Protocol is ISO14230_Slow");
        protocol = "ISO14230_Slow";
      }
      delay(30);
      SERIAL_OBD.write(~resultBuffer[2]);  //0xF7
      REQUEST_DELAY = 50;
      readData();
      REQUEST_DELAY = 500;
  ...
  if (protocol == "Automatic" || protocol == "ISO14230_Fast") {
    ...
    if (resultBuffer[8] == 0xC1) {
      SERIAL_DEBUG.println("Your Protocol is ISO14230_Fast");
...
void writeData(const byte data[], int length, const byte pid) {
  SERIAL_DEBUG.println("Writing Data");
  ...
  for (int i = 0; i < length + 2; i++) {
    SERIAL_OBD.write(extendedData[i]);
    delay(READ_DELAY);
  }
}
...
void readData() {
  delay(REQUEST_DELAY);
  int result = SERIAL_OBD.available();
  if (result > 0) {
    memset(resultBuffer, 0, sizeof(resultBuffer));
    SERIAL_DEBUG.print("Received Data: ");
    int bytesRead = min(result, int(sizeof(resultBuffer)));
    for (int i = 0; i < bytesRead; i++) {
      resultBuffer[i] = SERIAL_OBD.read();
      SERIAL_DEBUG.print(resultBuffer[i], HEX);
      SERIAL_DEBUG.print(" ");
      delay(READ_DELAY);
    }
    SERIAL_DEBUG.println();
